...
const mockVendedoras = [
  {
    ...
    historial: [
      { id: 1, monto: 63.0, estado: "pendiente", fecha: "25/8/2025, 7:13:33 p. m.", metodo: "transferencia", referencia: "-", notas: "-" },
      { id: 2, monto: 30.0, estado: "pagado", fecha: "24/8/2025, 6:00:00 p. m.", metodo: "efectivo", referencia: "-", notas: "-" },
    ],
  },
  {
    id: 2,
...
export default function PagoTab() {
  ...
  const [resumen, setResumen] = useState({ totalDeuda: 0, promedio: 0, conDeuda: 0 });
  ...
  const marcarPagado = (pagoId) => {
    ...
    setTimeout(() => {
      ...
      const deudaActualizada = historialActualizado.reduce((a, p) => p.estado === "pendiente" ? a + p.monto : a, 0);
      setSelectedVendedora({ ...selectedVendedora, historial: historialActualizado, deuda_actual: deudaActualizada });
      const vendedorasActualizadas = vendedoras.map(v =>
        v.id === selectedVendedora.id ? { ...v, deuda_actual: deudaActualizada } : v
      );
  ...
  const totalPendiente = selectedVendedora?.historial?.filter(p => p.estado === "pendiente").reduce((a,p)=>a+p.monto,0) || 0;
  const totalPagado = selectedVendedora?.historial?.filter(p => p.estado === "pagado").reduce((a,p)=>a+p.monto,0) || 0;
  ...
  return (
    <div className="p-4">
      ...
      {loading ? (
        <p>Cargando vendedoras...</p>
      ) : (
        <>
          <div className="mb-4 p-4 bg-gray-100 rounded flex flex-col gap-2">
      ...
      {selectedVendedora && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-start justify-center z-50 overflow-auto py-10">
          <div className="bg-white rounded-2xl w-11/12 max-w-6xl max-h-[90vh] overflow-y-auto p-6 relative shadow-2xl border border-gray-200">
            
            {/* Botón de cierre */}
            <button
              className="absolute top-5 right-5 text-gray-500 hover:text-gray-800 font-bold text-2xl"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 border-b pb-3">
              <div>
                <h3 className="font-bold text-2xl">{selectedVendedora.nombre}</h3>
                <p className="text-gray-600">
                  Deuda actual: <span className="font-semibold">${selectedVendedora.deuda_actual.toFixed(2)}</span>
                </p>
                <p className="text-gray-500 text-sm">
                  Última actualización: {selectedVendedora.ultima_actualizacion || "-"}
                </p>
              </div>
              <div className="bg-yellow-100 p-4 rounded flex flex-col items-center justify-center">
                ...
                <p className="text-xl font-bold">${totalPendiente.toFixed(2)}</p>
              </div>
                ...
                <table className="w-full text-sm border">
                  ...
                  <tbody>
                    {pagosFiltrados.map((pago) => (
                      <tr key={pago.id} className={pago.estado === "pendiente" ? "bg-yellow-100" : "bg-green-100"}>
                        <td className="p-2 border">{pago.id}</td>
                        <td className="p-2 border">${pago.monto.toFixed(2)}</td>
                        <td className="p-2 border font-semibold">{pago.estado}</td>
